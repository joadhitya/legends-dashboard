    ...
    <div class="card-header p-0 pb-2">
        <div class="toolbar row ">
            <div class="col-md-6 col-sm-6 col-6">
                <h5 class="h5 text-uppercase">List Konten Uji Kompetensi Setting</h5>
            </div>
        </div>
    </div>

    <table class="table datatables table-bordered table-hover table-data" id="table-competence-test">
        <thead>
        ...
        <tbody>
            @forelse ($data as $detail)
                <tr>
                    ...
                    <td>{{ $detail->name }}</td>
                    <td class="w-10p">{{ $detail->is_active=='yes' ? 'Aktif' : 'Tidak Aktif' }}</td>
                    <td class="w-5p">
                        ...
                        <div class="dropdown-menu dropdown-menu-right">
                            <label class="dropdown-item pointer"
                                onclick="activityDetail('competence-test','edit',{{ $detail->id }})"><span
                                    class="mr-2 fe fe-edit"></span>Edit</label>
                        </div>
                    </td>
                </tr>
            @empty
                <tr colspan="5">Tidak terdapat data konten Uji Kompetensi Perusahaan</tr>
            @endforelse
        </tbody>
    </table>
